 ...
 * @since 1.0
 *
 * @uses jquery_easy_menu
 */
class jquery_easy_menu_Admin extends jquery_easy_menu {
	/**
	 ...
	 * @uses do_action() Calls 'jqem_enqueue_scripts'.
	 */
	public function enqueue_scripts() {
		...
		// Enqueue old script
		wp_enqueue_script( 'jqem-scripts', plugins_url( 'jqem.js', __FILE__ ), array( 'media-upload', 'thickbox' ), '1', true );
		wp_localize_script( 'jqem-scripts', 'jqem_vars', array(
				'alert' => __( 'You need to set an image as a featured image to be able to use it as an menu item image', 'jqem' )
			)
		);
		add_thickbox();
		...
		if ( version_compare( $wp_version, '3.5', '>=' ) ) {
			wp_enqueue_media();
			wp_enqueue_script( 'jqem-media-view', plugins_url( 'media-view.js', __FILE__ ), array( 'jquery', 'media-editor', 'media-views', 'post' ), '1.1', true );
		}

		do_action( 'jqem_enqueue_scripts' );
	}
	...
	public function filter_walker( $walker ) {
		...
		return 'jqem_Walker_Nav_Menu_Edit';
	}
	 ...
	 * @uses apply_filters() Calls 'jqem_admin_post_thumbnail_html' to
	...
	public function _wp_post_thumbnail_html( $content, $post_id ) {
		...
		if ( ! isset( $_REQUEST['jqem_request'] ) )
			return $content;
		...
		if ( has_post_thumbnail( $post->ID ) )
			...
			$link = __( 'Upload menu item image', 'jqem' );
		...
		if ( isset( $_REQUEST['thumb_was'] ) && -1 == $_REQUEST['thumb_was'] ) {
			$link_text = __( 'Change menu item image', 'jqem' );
			$ajax_nonce = wp_create_nonce( 'set_post_thumbnail-' . $post->ID );
			$remove_link = ' | <a href="#" data-id="' . $post->ID . '" class="jqem_remove" onclick="jqemRemoveThumbnail(\'' . $ajax_nonce . '\',' . $post->ID . ');return false;">' . esc_html__( 'Remove menu item image', 'jqem' ) . '</a>';
		...
		return apply_filters( 'jqem_admin_post_thumbnail_html', $content, $post->ID );
	}
	 ...
	 * @uses apply_filters() Calls 'jqem_attachment_fields_to_edit' to
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		// Add "hover" checkbox
		$parent_hover_id = get_post_meta( $post->post_parent, '_jqem_hover', true );
		$is_hover = ( $parent_hover_id == $post->ID ) ? true : false;
		$is_hover_checked = checked( $is_hover, true, false );

		$form_fields['jqemhover'] = array(
			'label'        => __( 'Used on hover?', 'jqem' ),
			'input'        => 'html',
			'html'         => "<input type='checkbox' class='jqem-hover-checkbox' {$is_hover_checked} name='attachments[{$post->ID}][jqemhover]' id='attachments[{$post->ID}][jqemhover]' data-parent='{$post->post_parent}' data-checked='{$is_hover}' />",
			'value'        => $is_hover,
			'helps'        => __( 'Should this image be used on hover', 'jqem' ),
			'show_in_edit' => false
		);

		// Add "active" checkbox
		$parent_active_id = get_post_meta( $post->post_parent, '_jqem_active', true );
		$is_active = ( $parent_active_id == $post->ID ) ? true : false;
		$is_active_checked = checked( $is_active, true, false );

		$form_fields['jqemactive'] = array(
			'label'        => __( 'Used when active?', 'jqem' ),
			'input'        => 'html',
			'html'         => "<input type='checkbox' class='jqem-active-checkbox' {$is_active_checked} name='attachments[{$post->ID}][jqemactive]' id='attachments[{$post->ID}][jqemactive]' data-parent='{$post->post_parent}' data-checked='{$is_active}' />",
			'value'        => $is_active,
			'helps'        => __( 'Should this image be used when menu item is active', 'jqem' ),
			'show_in_edit' => false
		);

		// Filter returned HTML output
		return apply_filters( 'jqem_attachment_fields_to_edit', $form_fields, $post );
	}
	...
	function attachment_fields_to_save( $post, $attachment ) {
		// Save "hover" checkbox
		if ( 'on' == $attachment['jqemhover'] )
			update_post_meta( $post['post_parent'], '_jqem_hover', $post['ID'] );
		else
			delete_post_meta( $post['post_parent'], '_jqem_hover', $post['ID'] );

		// Save "active" checkbox
		if ( 'on' == $attachment['jqemactive'] )
			update_post_meta( $post['post_parent'], '_jqem_active', $post['ID'] );
		else
			delete_post_meta( $post['post_parent'], '_jqem_active', $post['ID'] );
