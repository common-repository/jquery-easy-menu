 ...
 */
class jqem_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	/**
	 ...
	 * @uses do_action_ref_array() Calls 'jqem_menu_item_walker_output' with the output.
	 *                        post object, depth and arguments to overwrite item's output.
	 * @uses jqem_Walker_Nav_Menu_Edit::get_settings() To get JSONed item's data.
	 * @uses do_action_ref_array() Calls 'jqem_menu_item_walker_end' with the output.
	 *                        post object, depth and arguments to overwrite item's output.
	 *
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$output .= '<input type="hidden" name="jqem_item_id" id="jqem_item_id" value="' . esc_attr( $item_id ) . '" />';

		// Generate menu item image & link's text string
		if ( has_post_thumbnail( $item_id ) ) {
			$post_thumbnail = get_the_post_thumbnail( $item_id, 'thumb' );
			$output .= '<div class="jqem-current-image jqem-div" style="display: none;"><a href="' . $upload_url . '" data-id="' . $item_id . '" class="thickbox add_media">' . $post_thumbnail . '</a></div>';
			...
			if ( version_compare( $wp_version, '3.5', '>=' ) ) {
				...
				$remove_link = ' | <a href="#" data-id="' . $item_id . '" class="jqem_remove" onclick="jqemRemoveThumbnail(\'' . $ajax_nonce . '\',' . $item_id . ');return false;">' . esc_html__( 'Remove menu item image', 'jqem' ) . '</a>';
			}
		} else {
			$output .= '<div class="jqem-current-image jqem-div" style="display: none;"></div>';
			$link_text = __( 'Upload menu item image', 'jqem' );
		}

		// Append menu item upload link
		$output .= '<div class="jqem-upload-link jqem-div" style="display: none;"><a href="' . $upload_url . '" data-id="' . $item_id . '" class="thickbox add_media">' . esc_html( $link_text ) . '</a>';
		...
		// Filter output
		do_action_ref_array( 'jqem_menu_item_walker_output', array( &$output, $item, $depth, $args ) );

		// Add JSONed meta data
		$output .= $this->get_settings( $item_id );

		do_action_ref_array( 'jqem_menu_item_walker_end', array( &$output, $item, $depth, $args ) );
	}
	 ...
	 * @uses do_action() Calls 'jqem_setup_settings_var' with the item ID.
	 *
	...
	public function get_settings( $post_id ) {
		...
		$object_name = 'jqem_settings[' . $post->ID . ']';
		...
		if ( ! did_action( 'jqem_setup_settings_var' ) ) {
			$script = "var jqem_settings = [];\n" . $script;
			do_action( 'jqem_setup_settings_var', $post->ID );
		}
